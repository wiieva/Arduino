...
static uint8_t get_width(const struct mf_bwfont_char_range_s *r, uint16_t index)
{
    ...
    {
        return gfxROMread8(r->glyph_widths+index);
    }
}
                           ...
                           void *state)
{
    ...
    {
        data = r->glyph_data + gfxROMread16(r->glyph_offsets + index) * r->height_bytes;
        num_cols = gfxROMread16(r->glyph_offsets + index + 1) - gfxROMread16(r->glyph_offsets + index);
    }
