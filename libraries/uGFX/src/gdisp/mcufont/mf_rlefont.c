                                 ...
                                 uint16_t character)
{
   ...
   for (i = 0; i < font->char_range_count; i++)
   {
       ...
       {
           uint16_t offset = gfxROMread16 (range->glyph_offsets + index);
...
static void write_rle_dictentry(const struct mf_rlefont_s *font,
                                ...
                                uint8_t index)
{
    uint16_t offset = gfxROMread16 (font->dictionary_offsets + index);
    uint16_t length = gfxROMread16 (font->dictionary_offsets + index + 1) - offset;
    uint16_t i;
    
    for (i = 0; i < length; i++)
    {
        uint8_t code = gfxROMread8 (font->dictionary_data+offset + i);
                                    ...
                                    void *state)
{
    ...
    {
        write_glyph_codeword((struct mf_rlefont_s*)font, &rstate, gfxROMread8(p++));
    }
