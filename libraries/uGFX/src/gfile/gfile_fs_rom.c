...
typedef struct ROMFS_DIRENTRY {
	uint32_t						ver;			// Directory Entry Version
	uint32_t						cmp;			// Compression format
...
static const ROMFS_DIRENTRY *ROMFindFile(const char *fname)
{
	...
	for(p = FsROMHead; p; p = p->next) {
		if (p->ver <= ROMFS_DIR_VER_MAX && p->cmp == ROMFS_CMP_UNCOMPRESSED && !gfxROMstrcmp(p->name, fname))
			break;
	}
	return p;
}
...
static int ROMRead(GFILE *f, void *buf, int size)
{
	...
	if (p->size - f->pos < size)
		size = p->size - f->pos;
	if (size <= 0)	return 0;
	gfxROMmemcpy(buf, p->file+f->pos, size);
	return size;
}
